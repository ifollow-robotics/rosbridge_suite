...
__copyright__ = "Copyright 2014-2015 Brian Olson"
...
This source file has been modified to accept byte arrays.
"""
...
if _IS_PY3:
    def _is_byte_array(val):
        return isinstance(val, (bytes, bytearray))
else:
    def _is_byte_array(val):
        # str is a subclass of bytes
        return isinstance(val, bytearray) or (not _is_stringish(val) and isinstance(val, bytes))
...
def dumps_byte_array(arr):
    head = _encode_type_num(CBOR_BYTES, len(arr))
    return head + arr
...
if _IS_PY3:
    ...
    def dumps_dict(d, sort_keys=False):
        enc = _encode_type_num(CBOR_MAP, len(d))
        if sort_keys:
            for k in sorted(d.iterkeys()):
                v = d[k]
                enc += dumps(k, sort_keys=sort_keys)
                enc += dumps(v, sort_keys=sort_keys)
        else:
...
if _IS_PY3:
    ...
    def _is_stringish(x):
        return isinstance(x, (str, basestring, unicode))
    def _is_intish(x):
...
def dumps(ob, sort_keys=False):
    ...
    if _is_byte_array(ob):
        return dumps_byte_array(ob)
    if _is_stringish(ob):
